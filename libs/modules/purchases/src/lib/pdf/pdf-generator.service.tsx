...
/** Picks one of the 5 visual templates (user's own default preference,
 * see PurchasePreferencesService, overridable per-download) and renders it
 * to a PDF buffer - same data shape for all 5, see pdf-data.ts. */
@Injectable()
export class PdfGeneratorService {
  async generate(style: PdfStyle, data: PurchaseDocumentPdfData): Promise<Buffer> {
    return renderToBuffer(this.pickTemplate(style, data));
  }

  private pickTemplate(style: PdfStyle, data: PurchaseDocumentPdfData) {
    switch (style) {
      case 'MODERNO':
