...
import type { CustomerStatement, CustomerStatementEntry } from '../receivables.service.js';

interface ColumnDef {
  header: string;
  width: number;
  value: (e: CustomerStatementEntry) => string | number;
  isAmount?: boolean;
}
...
@Injectable()
export class CustomerStatementExcelService {
  async generate(statement: CustomerStatement): Promise<Buffer> {
    const workbook = new ExcelJS.Workbook();
    const sheet = workbook.addWorksheet('Cuenta Corriente'.slice(0, 31));

    sheet.mergeCells(1, 1, 1, COLUMNS.length);
    const titleCell = sheet.getCell(1, 1);
    titleCell.value = `Cuenta Corriente - ${statement.customerName}`;
