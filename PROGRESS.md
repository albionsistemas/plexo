...
- Base `plexo` creada, las 8 migraciones aplicadas (la última, `list_tenant_ids_function`, agregada 2026-07-21), seed corrido.
...
2. Frontend: siguen faltando las pantallas de Facturación, Cuentas a Cobrar, Contabilidad, Impuestos y Reportes — Inventario fue la primera. Corregir además el mojibake pre-existente en nombres con tildes (p. ej. "Dep�sito Central") — es un problema de encoding en los datos ya sembrados, no del código.
3. Arreglar los 2 tests preexistentes rotos que quedan (ver nota arriba).
