...
- `invoicing:lint`: error de `@nx/dependency-checks` — usa `@nestjs/event-emitter` pero no está en `package.json` de esa lib.
...
2. Cron jobs reales (NestJS `@nestjs/schedule` o similar) — `refreshOverdueStatuses` de Cuentas a Cobrar es manual por ahora.
3. Integración AFIP real (hoy: CAE falso, `StubElectronicInvoicingService`).
4. Frontend: siguen faltando las pantallas de Facturación, Cuentas a Cobrar, Contabilidad, Impuestos y Reportes — Inventario fue la primera. Corregir además el mojibake pre-existente en nombres con tildes (p. ej. "Dep�sito Central") — es un problema de encoding en los datos ya sembrados, no del código.
5. Arreglar los 3 bugs/tests preexistentes rotos (ver nota arriba).
6. Notas de crédito no reponen stock (ver nota arriba).
