  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="rounded-xl border border-slate-800 bg-slate-900 p-4">
        <h2 className="mb-3 text-sm font-semibold text-slate-300">Ventas por cliente</h2>
        {byCustomerQuery.isLoading ? (
          <div className="flex h-24 items-center justify-center text-slate-500">Cargando...</div>
        ) : byCustomer.length === 0 ? (
          <p className="text-sm text-slate-600">Sin ventas en el período</p>
        ) : (
          <table className="w-full text-sm">
            ...
            <tbody>
              {byCustomer.map((row) => (
                <tr key={row.customerId} className="border-b border-slate-800/50">
                  <td className="py-2 pr-4 text-slate-200">{row.customerName}</td>
                  <td className="py-2 pr-4 text-right text-slate-400">{row.invoiceCount}</td>
      ...
      <div className="rounded-xl border border-slate-800 bg-slate-900 p-4">
        <h2 className="mb-3 text-sm font-semibold text-slate-300">Ventas por producto</h2>
        {byProductQuery.isLoading ? (
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {byProduct.map((row) => (
                <tr key={row.articleVariantId} className="border-b border-slate-800/50">
                  <td className="py-2 pr-4 font-mono text-xs text-slate-400">{row.sku}</td>
                  <td className="py-2 pr-4 text-slate-200">{row.articleName}</td>
                  <td className="py-2 pr-4 text-right text-slate-400">{row.quantitySold}</td>
                  <td className="py-2 text-right font-medium text-slate-100">
