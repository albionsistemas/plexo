    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-4">
        <div className="rounded-xl border border-slate-800 bg-slate-900 p-4">
          ...
          <p className="mt-1 text-lg font-semibold text-slate-100">{summary?.invoiceCount ?? '—'}</p>
        </div>
        <div className="rounded-xl border border-slate-800 bg-slate-900 p-4">
      ...
      <div className="rounded-xl border border-slate-800 bg-slate-900 p-4">
        <h2 className="mb-3 text-sm font-semibold text-slate-300">Estado de resultados (libro mayor)</h2>
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {statement?.lines.map((line) => (
                  <tr key={line.accountId} className="border-b border-slate-800/50">
                    <td className="py-2 pr-4 font-mono text-xs text-slate-400">{line.code}</td>
                    <td className="py-2 pr-4 text-slate-200">{line.name}</td>
                    <td className="py-2 pr-4 text-slate-400">{TYPE_LABELS[line.type]}</td>
                    <td className="py-2 text-right text-slate-300">${Number(line.amount).toFixed(2)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
            <div className="mt-4 flex flex-col gap-1 border-t border-slate-800 pt-3 text-sm">
