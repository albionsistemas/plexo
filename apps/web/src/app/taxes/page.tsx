  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex items-center justify-between">
        <h1 className="text-xl font-semibold text-slate-100">Impuestos</h1>
        <button
      ...
      <div className="rounded-xl border border-slate-800 bg-slate-900 p-4">
        {isLoading ? (
          ...
          <p className="text-sm text-slate-600">Sin impuestos definidos</p>
        ) : (
          <table className="w-full text-sm">
            ...
            <tbody>
              {sorted.map((def) => {
                ...
                return (
                  <tr key={def.id} className="border-b border-slate-800/50">
                    <td className="py-2 pr-4 font-mono text-xs text-slate-400">{def.code}</td>
                    <td className="py-2 pr-4 text-slate-200">{def.name}</td>
                    <td className="py-2 pr-4 text-slate-400">{CALC_TYPE_LABELS[def.calculationType]}</td>
                    <td className="py-2 pr-4 text-slate-300">{formatValue(def)}</td>
